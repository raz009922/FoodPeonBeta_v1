...
import com.example.dcastalia.rrrrrr.API.Retrofit3;
import com.example.dcastalia.rrrrrr.Adapter.MyAdapter;
import com.example.dcastalia.rrrrrr.Adapter.MyAdapter1;
...
public class FoodListActivity extends AppCompatActivity /*implements SearchView.OnQueryTextListener*/ {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setupRecyclerView();
        SharedPreferences preferences = PreferenceManager.getDefaultSharedPreferences(FoodListActivity.this);
      ...
      /*  prepareActionBar(toolbar);*/
        ...
        Retrofit3 retroFIT=retrofit.create(Retrofit3.class);

        Call<List<FoodList>> call =  retroFIT.getFood();

        //calling the api. it need two callback one is response and the other is failure

        call.enqueue(new Callback<List<FoodList>>() {
            @Override
            public void onResponse(Call<List<FoodList>> call, final Response<List<FoodList>> response) {
                Log.i("JSON","yahoooaa  "+response);

                // Log.i(""+response,"JSON");
                final List<FoodList> foodLists=response.body();


                adapter=new MyAdapter1(foodLists,getApplicationContext());
                recyclerView.setAdapter(adapter);
                SwipeHelper swipeHelper = new SwipeHelper(FoodListActivity.this, recyclerView) {
                    @Override
                                ...
                                new UnderlayButtonClickListener() {
                                    ...
                                    public void onClick(int pos) {
                                        ...
                                        if(kitchenID>0){
                                            ...
                                            sqliteDatabaseAdapter=new SqliteDatabaseAdapter(FoodListActivity.this);
